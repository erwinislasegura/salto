...
<div class="crm-page-head">
    <div>
        <p class="crm-eyebrow">Usuarios</p>
        <h1><?= $isEdit ? 'Editar usuario' : 'Nuevo usuario' ?></h1>
        <p><?= $isEdit ? 'Actualiza datos, rol o estado del acceso.' : 'Crea una cuenta interna para administrar el CRM.' ?></p>
    </div>
</div>
<div class="crm-card">
    <form method="post" action="index.php?controller=users&action=<?= $isEdit ? 'update' : 'store' ?>" class="row g-3">
        ...
        <div class="col-12 d-flex gap-2 pt-2"><button class="btn btn-primary">Guardar</button><a class="btn btn-outline-secondary" href="index.php?controller=users">Cancelar</a></div>
    </form>
</div>
